...
# NEW ADDITION: optional deep-learning classifiers (imports may fail if deps/models missing)
try:
    from dl_intent_classifier import predict_intent
    from dl_document_classifier import classify_document
...
def generate_response(
    ...
    # NEW ADDITION: optional intent line before Context
    intent_context = ""
    if intent_label:
        c = confidence if confidence is not None else 0.0
        intent_context = f"Detected Legal Intent: {intent_label} (confidence: {c:.2f})\n\n"
    prompt = f"""
...
def handle_chat(service: str):
    ...
    # NEW ADDITION: DL intent classification
    intent_label = None
    confidence = None
    if DL_READY:
        try:
    ...
    try:
        reply = generate_response(
            query, contexts, history, service, intent_label, confidence
        )
...
# NEW ADDITION: DL architecture metadata
@app.route("/dl-info", methods=["GET"])
def dl_info():
    return jsonify(
        {
            "dl_architecture": {
                "intent_classifier": {
                    ...
                    "type": "LegalBERT + Custom MLP Head",
                    "architecture": "Linear(768→512)→ReLU→Dropout→Linear(512→256)→ReLU→Dropout→Linear(256→3)",
                    "classes": [
                        "personal-and-family-legal-assistance",
                        "business-consumer-and-criminal-legal-assistance",
                        "consultation",
                    ],
                    "status": "loaded" if DL_READY else "not trained yet",
                },
                "document_classifier": {
                    "model": "bert-base-uncased",
                    "type": "BERT + Mean Pooling + Custom MLP Head",
                    "architecture": "Linear(768→512)→BatchNorm→ReLU→Dropout→Linear(512→128)→ReLU→Dropout→Linear(128→7)",
                    "classes": [
                        "petition",
                        "judgment",
                        "contract",
                ...
                "pipeline": "User Query → DL Intent Classification → RAG Retrieval → Groq LLM → Response",
            }
        }
    )


# NEW ADDITION: train intent model on sample data (requires torch/transformers)
@app.route("/train-models", methods=["POST"])
def train_models():
    try:
        from dl_training_pipeline import (
            TrainingConfig,
            generate_sample_legal_data,
            run_training_pipeline,
        )
        from dl_intent_classifier import LegalIntentClassifier

        texts, labels = generate_sample_legal_data()
        model = LegalIntentClassifier()
